...
export function BookingRequestForm({
  ...
  const estimatedPrice = (() => {
    switch (duration) {
      case '4':
        return priceRates.price_4h;
      case '6':
        return priceRates.price_6h;
      case '8':
        return priceRates.price_8h;
      default:
        return 0;
    }
  })();
  ...
  async function onSubmit(data: BookingFormValues) {
    ...
    try {
      ...
      const endAt = new Date(
        startAt.getTime() + parseInt(data.duration) * 60 * 60 * 1000,
      );
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="p-4 bg-muted/20 rounded-lg flex justify-between items-center">
          ...
          <span className="text-xl font-bold">
            {estimatedPrice} {priceRates.currency}
          </span>
        </div>
